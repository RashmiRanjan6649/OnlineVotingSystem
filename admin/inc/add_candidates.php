...
{?>
 ...
 Candidate image is too large. Please Upload Small File(You can upload any image upto 2mbas.)
</div>
 <?php
...
?>
<div class="alert alert-danger my-3" role="alert">
 Invalid Image Type (Only .jpg .png .jpeg type are allowed)
</div>
<?php
...
{   
    $c_id = $_GET['delete_Candidate'];
    mysqli_query($db,"DELETE FROM candidate_details WHERE id='".$c_id."'") or die(mysqli_error($db));
    ?>
    <div class="alert alert-danger my-3" role="alert">
...
<div class="row my-3">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <select class="form-control" name="election_id" required>
                ...
                if($isAnyElectionAdded>0)
                {
                    while ($row=mysqli_fetch_assoc($fetchingElections))
                     {
                        ...
                        $fetchingCandidates = mysqli_query($db, "SELECT *from candidate_details WHERE election_id='".$election_id."'") or die(mysqli_errno($db));
                    ...
                    ?>
                    <option>No any Election Is Added</option>
                    <?php
                }
                ?>
            </select>
            ...
            <input type="submit" value="Add Candidate" name="addCandidateBtn" class="btn btn-success" style="background-color:rgb(54,54,199);"/>
      </form>
    </div>
    ...
    <tbody>
        <?php
        $fetchingData = mysqli_query($db, "SELECT *FROM candidate_details") or die();
        ...
        {   $sno=1;
            while ($row=mysqli_fetch_array($fetchingData))
             {
              ...
              //  echo $candidate_photo;
                ...
                <tr>
                    <td> <?php echo $sno++; ?></td>
                    <td><img src="<?php echo $row['candidate_photo']?>" class="candidate_photos"/> </td>
                    <td><?php echo $row['candidate_name']?></td>
                    <td><?php echo $row['candidate_details']?></td>
                    <td><?php echo $election_name?></td>
                   
                    <!-- <td><a href="#" class="btn btn-sm btn-warning">Edit</a></td> -->
                    <td><button class="btn btn-sm btn-danger"  onclick="DeleteCandidate(<?php echo $candidate_id; ?>);" >Delete</button></td>
                </tr>
                <?php
            }
        }
            ...
            ?>
            <tr>
                <td colspan="7">No any Candidate is adde yet</td>
            </tr>
            <?php
        }
        ?>
